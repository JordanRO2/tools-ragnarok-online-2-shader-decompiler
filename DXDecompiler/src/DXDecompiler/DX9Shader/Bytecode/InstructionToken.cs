...
namespace DXDecompiler.DX9Shader
{
	...
	public class InstructionToken : Token
	{
		...
		{
			int swizzleLength = 4;
			// Source component -> swizzle slot mapping. Identity by default,
			// but for a masked HLSL destination the source components must be
			// aligned to the destination write-mask channels (D3D9 evaluates
			// masked ALU ops per channel). Without this a non-prefix mask like
			// .yzw emits the wrong swizzle (e.g. .wzy instead of .zyx), which
			// also fails to compile when the value is narrower than 4 components.
			...
			else if(hlsl)
			{
				...
				else if(HasDestination)
				{
					...
					swizzleLength = sourceSlots.Length;
				}
			}
		...
		public string GetDeclSemantic()
		{
			...
			switch(registerType)
			{
				...
				case RegisterType.Output when _shaderModel.Type == ShaderType.Vertex && _shaderModel.MajorVersion >= 3:
					// In pixel shaders SM1-2 the only input (v#) registers are the color
					// registers (v0=COLOR0, v1=COLOR1); texture coords use t# registers.
					// Their dcl usage (e.g. dcl_position v0) is not a legal ps_2_0 input
					// semantic in HLSL - fxc rejects it ("invalid ps_2_0 input semantic
					// 'POSITION'"). Map these inputs to COLOR+index, which matches the
					// vertex shader's oD# (COLOR) outputs so the VS/PS link stays valid.
					if(registerType == RegisterType.Input
						&& _shaderModel.Type == ShaderType.Pixel
						&& _shaderModel.MajorVersion < 3)
					{
						var colorIndex = GetParamRegisterNumber(1);
						return "COLOR" + (colorIndex == 0 ? string.Empty : colorIndex.ToString());
					}
					string name;
					switch(GetDeclUsage())
					{
